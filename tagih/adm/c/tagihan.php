...
if ($_POST['btnSMP2'])
	...
	$tgl2_bln = trim($tgl2_pecahku[1]);
	$tgl2_thn = trim($tgl2_pecahku[0]);
	$e_tgl2 = "$tgl2_thn:$tgl2_bln:$tgl2_tgl";
	...
	$e_diskon = cegah($_POST['e_diskon']);
	$e_ppn = cegah($_POST['e_ppn']);
	$e_pajak1 = cegah($_POST['e_pajak1']);
	$e_pajak2 = cegah($_POST['e_pajak2']);
	...
	//hitung ulang
	$e_total = ($e_subtotal + $e_pajak1 + $e_pajak2 + $e_ppn) - $e_diskon;  


		
	//update
	mysqli_query($koneksi, "UPDATE user_tagihan SET discount = '$e_diskon', ".
								"ppn = '$e_ppn', ".
								"total = '$e_total', ".
								"tgl_tagihan = '$e_tgl1', ".
								"tgl_expire = '$e_tgl2' ".
...
if ($s == "detail")
	...
	$(document).ready(function () {
		...
		$('#e_diskon').bind('keyup paste', function(){
			this.value = this.value.replace(/[^0-9]/g, '');
			});
			
		$('#e_ppn').bind('keyup paste', function(){
			this.value = this.value.replace(/[^0-9]/g, '');
			});

		$('#e_pajak1').bind('keyup paste', function(){
			this.value = this.value.replace(/[^0-9]/g, '');
			});

		$('#e_pajak2').bind('keyup paste', function(){
			this.value = this.value.replace(/[^0-9]/g, '');
			});
	    ...
	    $('#e_diskon').on("keyup", function () {
			hitungkabeh();
	    })


											
	    $('#e_ppn').on("keyup", function () {
			hitungkabeh();
	    })
	    ...
	    function hitungkabeh() {
            	var e_nil1 = parseInt($('#e_subtotal').val());
            	var e_nil2 = parseInt($('#e_diskon').val());
            	var e_nil3 = parseInt($('#e_ppn').val());
            	var e_nil4 = parseInt($('#e_pajak1').val());
            	var e_nil5 = parseInt($('#e_pajak2').val());
            	
	            totalnya = (e_nil1 + e_nil3 + e_nil4 + e_nil5) - e_nil2;
	...
	mysqli_query($koneksi, "DELETE FROM user_tagihan_item ".
								"WHERE user_kd = '$kdx' ".
								"AND periode_bln = '$ubln' ".
								"AND periode_thn = '$uthn'");
	...
	echo '<div class="table-responsive">          
		...
		if (!empty($tst))
			...
			do
				...
				//cek dulu, jika null, entri baru
				$qcc2 = mysqli_query($koneksi, "SELECT * FROM user_tagihan_item ".
													"WHERE user_kd = '$kdx' ".
													"AND periode_bln = '$ubln' ".
													"AND periode_thn = '$uthn' ".
													"AND item_kd = '$st_itemkd'");
				$tcc2 = mysqli_num_rows($qcc2);
				...
				if (empty($tcc2))
					...
					$xyz = "$x$nomer";
					//insert
					mysqli_query($koneksi, "INSERT INTO user_tagihan_item(kd, user_kd, user_kode, user_nama, ".
											"user_alamat, user_koordinat, user_telp, user_wa, ".
											"periode_bln, periode_thn, item_kd, item_kode, item_nama, ".
											"item_satuan, item_harga, harga, ".
											"qty, subtotal, postdate) VALUES ".
											"('$xyz', '$uskd', '$e_kode', '$e_nama', ".
											"'$e_alamat', '$e_koordinat', '$e_telp', '$e_wa', ".
											"'$ubln', '$uthn', '$st_kd', '$st_kode', '$st_nama2', ".
											"'$st_satuan2', '$st_harga2', '$st_harga2', ".
											"'$st_qty2', '$st_subtotal2', '$today')");
					}
		...
		//update
		mysqli_query($koneksi, "UPDATE user_tagihan SET subtotal = '$yuk_subtotal', ".
									"pajak1 = '$yuk_pajak1', ".
									"pajak2 = '$yuk_pajak2' ".
									"WHERE user_kd = '$uskd' ".
									"AND periode_bln = '$ubln' ".
        ...
        <span class="info-box-number">
			...
			<div class="row">
				...
				<div class="col-md-4">
						...
						Diskon :
						<br>
						<input name="e_diskon" id="e_diskon" type="text" value="'.$yuk2_diskon.'" size="10" class="btn btn-block btn-warning" required>
						</p>
						...
						PPN 10% :
						<br>
						<input name="e_ppn" id="e_ppn" type="text" value="'.$yuk2_ppn.'" size="10" class="btn btn-block btn-warning" required>
						</p>
	...
	<a href="tagihan_pdf.php?tkd='.$yuk2_kd.'" target="_blank" class="btn btn-block btn-danger">PRINT PDF INVOICE >></a>
	...
	if (empty($ubln))
		...
		echo '[<font color="red"><b>'.$jml_belum.'</b></font> Belum Dibuat Tagihan]. 
		
		[<font color="red"><b>'.$jml_belum_bayar.'</b></font> Belum Dibayar]. 
			...
			do 
				...
				if (empty($tcc1))
					...
					if ($cc1_status == "TERBAYAR")
						{
						$cc1_ket = "<font color='green'><b>TERBAYAR</b></font>";
						}
					else
						{
