...
class PagingGoogle{
	...
	function navPage($activePage, $totalPage){
		...
		if($activePage > 1){
			$prev = $activePage - 1;
			$pageLink .= "<a href='paging2.php?page=1'><< First </a>
		                    | <a href='paging2.php?page=$prev'>< Prev </a> ";
		}
		else{ 
			$pageLink .= "<< First < Prev";
		}
		...
		for ($i = $activePage-2; $i<$activePage; $i++){
			if ($i < 1)
				continue;
				$numeric .= " | <a href='paging2.php?page=$i'>$i</a> | ";
		}
		$numeric .= " $activePage ";
		...
		$numeric .= ($activePage+2<$totalPage ? " ... <a href='paging2.php?page=$totalPage'>$totalPage</a> " : " ");
		$pageLink .= "$numeric";
				
		// buat next page dan last page
		if($activePage < $totalPage){
			$next = $activePage+1;
			$pageLink .= " | <a href='paging2.php?page=$next'>Next ></a>
						| <a href='paging2.php?page=$totalPage'>Last >></a> ";
		}
		else{
			$pageLink .= "Next > Last >>";
		}
