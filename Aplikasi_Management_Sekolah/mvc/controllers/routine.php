...
*/
	...
	function exist_teacher() {
		...
		if((int)$id) {
			...
			if(count($routine)) {
				$this->form_validation->set_message("exist_teacher", "%s already assign another schedule in this time.");
				return FALSE;
			}
			return TRUE;
	...
	function allschoolyear() {
		if($this->input->post('schoolyearID') == 0) {
			$this->form_validation->set_message("allschoolyear", "The %s field is required");
	     	return FALSE;
		}
		return TRUE;
	}
